...
public class EjecucionDeMantenimiento {
    ...
    @Embedded private Kilometraje kilometraje;
    ...
    @Embedded private LiquidacionDeServicio liquidacion;

    @Embedded private ProgramacionDeProximoServicio proximoServicio;
    ...
    @ElementCollection
    @CollectionTable(
            name = "observaciones_preventivas",
    ...
    public EjecucionDeMantenimiento(
            Long ordenId,
            Long tecnicoUsuarioId,
        ...
        if (tecnicoUsuarioId == null) {
            throw new IllegalArgumentException("El tecnico responsable es obligatorio");
        }
        if (kilometraje == null) {
        ...
        if (serviciosAutorizados != null) {
            this.serviciosAutorizados.addAll(serviciosAutorizados);
        }
    ...
    public void iniciar(boolean ordenAutorizadaYEmitida, boolean asignacionValida) {
        asegurarNoCerrada();
        if (!ordenAutorizadaYEmitida) {
            throw new IllegalStateException("La orden debe estar autorizada y emitida");
        }
        if (!asignacionValida) {
            throw new IllegalStateException(
                    "El tecnico no tiene una asignacion valida para la orden");
        }
        if (estado != EstadoDeEjecucion.PENDIENTE && estado != EstadoDeEjecucion.SUSPENDIDA) {
            throw new IllegalStateException(
                    "Solo una ejecucion PENDIENTE o SUSPENDIDA puede iniciarse");
        }
        if (inicio == null) {
    ...
    public void suspender() {
        asegurarNoCerrada();
        if (estado != EstadoDeEjecucion.EN_CURSO) {
            throw new IllegalStateException("Solo una ejecucion EN_CURSO puede suspenderse");
        }
    ...
    public TareaEjecutada registrarTarea(Long servicioId, TipoDeTarea tipo) {
        ...
        ServicioAutorizado autorizado =
                serviciosAutorizados.stream()
                        .filter(servicio -> servicio.getServicioId().equals(servicioId))
                        .findFirst()
                        .orElseThrow(
                                () ->
                                        new IllegalArgumentException(
                                                "La tarea no corresponde a un servicio autorizado"));
    ...
    public void iniciarTarea(Long tareaId) {
        exigirEnCurso();
        buscarTarea(tareaId).iniciar();
    }

    public void completarTarea(Long tareaId) {
        exigirEnCurso();
        buscarTarea(tareaId).completar();
    }

    public void marcarTareaNoRealizada(Long tareaId) {
        exigirEnCurso();
        buscarTarea(tareaId).marcarNoRealizada();
    }

    public ConsumoReal registrarConsumo(Long productoId, CantidadDeProducto cantidad) {
        exigirEnCurso();
        ProductoAutorizado autorizado =
                productosAutorizados.stream()
                        .filter(producto -> producto.getProductoId().equals(productoId))
                        .findFirst()
                        .orElseThrow(
                                () ->
                                        new IllegalArgumentException(
                                                "El producto consumido no fue autorizado"));
        ConsumoReal consumo = new ConsumoReal(productoId, cantidad, autorizado.getPrecio());
        consumos.add(consumo);
        return consumo;
    }

    public ResiduoDeclarado declararResiduo(
            Long tareaId, TipoDeResiduoGenerado tipo, VolumenDeclarado volumen) {
        exigirEnCurso();
        TareaEjecutada tarea = buscarTarea(tareaId);
        if (tarea.getEstado() != EstadoDeTarea.COMPLETADA || !tarea.getTipo().generaResiduo()) {
            throw new IllegalStateException(
                    "Solo una tarea completada que genere residuos puede tener una declaracion");
        }
    ...
    public void definirProximoServicio(ProgramacionDeProximoServicio programacion) {
        ...
        if (programacion == null) {
            throw new IllegalArgumentException("La programacion es obligatoria");
        }
    ...
    public void validarCierre() {
        ...
        boolean faltaResiduo =
                tareas.stream()
                        .filter(
                                tarea ->
                                        tarea.getEstado() == EstadoDeTarea.COMPLETADA
                                                && tarea.getTipo().generaResiduo())
                        .anyMatch(
                                tarea ->
                                        residuos.stream()
                                                .noneMatch(
                                                        residuo ->
                                                                residuo.getTareaId()
                                                                        .equals(tarea.getId())));
        if (faltaResiduo) {
            throw new IllegalStateException("Falta declarar el residuo de una tarea que lo genera");
        }
        if (proximoServicio == null) {
            throw new IllegalStateException("Debe definirse el proximo servicio antes de cerrar");
        }
    }
    ...
    private String determinarMoneda() {
        if (!serviciosAutorizados.isEmpty()) {
            return serviciosAutorizados.get(0).getPrecio().getMoneda();
        }
        if (!productosAutorizados.isEmpty()) {
            return productosAutorizados.get(0).getPrecio().getMoneda();
        }
        return "PEN";
    }
    ...
    private TareaEjecutada buscarTarea(Long tareaId) {
        return tareas.stream()
                ...
                .orElseThrow(
                        () ->
                                new IllegalArgumentException(
                                        "No se encontro la tarea con ID: " + tareaId));
    }

    private void exigirEnCurso() {
        asegurarNoCerrada();
        if (estado != EstadoDeEjecucion.EN_CURSO) {
            throw new IllegalStateException("La ejecucion debe estar EN_CURSO");
        }
    }

    private void asegurarNoCerrada() {
        if (estado == EstadoDeEjecucion.CERRADA) {
            throw new IllegalStateException("Una ejecucion CERRADA es inmutable");
        }
    }
    ...
    public Long getTecnicoUsuarioId() {
        return tecnicoUsuarioId;
    }
    ...
    public LiquidacionDeServicio getLiquidacion() {
        return liquidacion;
    }

    public ProgramacionDeProximoServicio getProximoServicio() {
        return proximoServicio;
    }

    public List<TareaEjecutada> getTareas() {
        return Collections.unmodifiableList(tareas);
    }

    public List<ConsumoReal> getConsumos() {
        return Collections.unmodifiableList(consumos);
    }

    public List<ResiduoDeclarado> getResiduos() {
        return Collections.unmodifiableList(residuos);
    }

    public List<ObservacionPreventiva> getObservaciones() {
        return Collections.unmodifiableList(observaciones);
    }

    public List<ServicioAutorizado> getServiciosAutorizados() {
        return Collections.unmodifiableList(serviciosAutorizados);
    }

    public List<ProductoAutorizado> getProductosAutorizados() {
        return Collections.unmodifiableList(productosAutorizados);
    }
}
