...
public class ResiduoDeclarado {
    ...
    @AttributeOverride(
            name = "valor",
            column = @Column(name = "volumen", nullable = false, precision = 12, scale = 3))
    @AttributeOverride(
            name = "unidad",
    ...
    public ResiduoDeclarado(Long tareaId, TipoDeResiduoGenerado tipo, VolumenDeclarado volumen) {
        if (tareaId == null || tipo == null || volumen == null) {
            throw new IllegalArgumentException(
                    "La tarea, el tipo y el volumen del residuo son obligatorios");
        }
    ...
    public TipoDeResiduoGenerado getTipo() {
        return tipo;
    }

    public VolumenDeclarado getVolumen() {
        return volumen;
    }

    public LocalDateTime getMomentoDeclaracion() {
        return momentoDeclaracion;
    }
}
