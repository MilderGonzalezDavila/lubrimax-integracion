...
public class TareaEjecutada {
    ...
    @AttributeOverride(
            name = "monto",
            column =
                    @Column(name = "precio_mano_obra", nullable = false, precision = 12, scale = 2))
    @AttributeOverride(
            name = "moneda",
            column = @Column(name = "moneda_mano_obra", nullable = false, length = 3))
    @AttributeOverride(
            name = "momento",
            column = @Column(name = "momento_precio_mano_obra", nullable = false))
    ...
    public TareaEjecutada(Long servicioId, TipoDeTarea tipo, PrecioAplicado precioAplicado) {
        ...
        if (precioAplicado == null)
            throw new IllegalArgumentException("El precio autorizado es obligatorio");
    ...
    public void iniciar() {
        if (estado != EstadoDeTarea.PENDIENTE)
            throw new IllegalStateException("Solo una tarea PENDIENTE puede iniciarse");
    ...
    public void completar() {
        if (estado != EstadoDeTarea.EN_CURSO)
            throw new IllegalStateException("Solo una tarea EN_CURSO puede completarse");
    ...
    public void marcarNoRealizada() {
        if (estado == EstadoDeTarea.COMPLETADA)
            throw new IllegalStateException(
                    "Una tarea COMPLETADA no puede marcarse como no realizada");
