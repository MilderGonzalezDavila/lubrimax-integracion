...
public class EjecucionDeMantenimientoServiceImpl implements EjecucionDeMantenimientoService {
    ...
    public EjecucionDeMantenimientoServiceImpl(
            EjecucionDeMantenimientoRepository repository,
    ...
    public EjecucionDeMantenimiento crear(Long ordenId, Long tecnicoUsuarioId, Long kilometraje) {
        if (repository.existsById(ordenId))
            throw new IllegalArgumentException("Ya existe una ejecucion para la orden");
        var orden = ordenCliente.obtenerParaEjecucion(ordenId);
        if (!orden.autorizadaYEmitida())
            throw new IllegalStateException("La orden no esta autorizada y emitida");
        ...
        List<ServicioAutorizado> servicios =
                listaSegura(orden.servicios()).stream()
                        .map(
                                servicio ->
                                        new ServicioAutorizado(
                                                servicio.servicioId(), precio(servicio.precio())))
                        .toList();
        List<ProductoAutorizado> productos =
                listaSegura(orden.productos()).stream()
                        .map(
                                producto ->
                                        new ProductoAutorizado(
                                                producto.productoId(), precio(producto.precio())))
                        .toList();
        return repository.save(
                new EjecucionDeMantenimiento(
                        ordenId,
                        tecnicoUsuarioId,
                        new Kilometraje(kilometraje),
                        servicios,
                        productos));
    }
                ...
                .orElseThrow(
                        () ->
                                new IllegalArgumentException(
                                        "No se encontro la ejecucion de la orden: " + ordenId));
    }

    @Override
    @Transactional(readOnly = true)
    public List<EjecucionDeMantenimiento> listar() {
        return (List<EjecucionDeMantenimiento>) repository.findAll();
    }
    ...
    public EjecucionDeMantenimiento iniciar(Long ordenId) {
        ...
        boolean asignacionValida =
                agendaCliente.validarAsignacion(ejecucion.getTecnicoUsuarioId(), ordenId).valida();
    ...
    public EjecucionDeMantenimiento registrarTarea(
            Long ordenId, Long servicioId, TipoDeTarea tipo) {
        EjecucionDeMantenimiento ejecucion = obtener(ordenId);
    ...
    public EjecucionDeMantenimiento registrarConsumo(
            Long ordenId, Long productoId, CantidadDeProducto cantidad) {
        EjecucionDeMantenimiento ejecucion = obtener(ordenId);
    ...
    public EjecucionDeMantenimiento declararResiduo(
            Long ordenId, Long tareaId, TipoDeResiduoGenerado tipo, VolumenDeclarado volumen) {
        EjecucionDeMantenimiento ejecucion = obtener(ordenId);
    ...
    public EjecucionDeMantenimiento registrarObservacion(
            Long ordenId, ObservacionPreventiva observacion) {
        EjecucionDeMantenimiento ejecucion = obtener(ordenId);
    ...
    public EjecucionDeMantenimiento definirProximoServicio(
            Long ordenId, ProgramacionDeProximoServicio programacion) {
        EjecucionDeMantenimiento ejecucion = obtener(ordenId);
    ...
    public EjecucionDeMantenimiento cerrar(Long ordenId) {
        ...
        ejecucion
                ...
                .forEach(
                        residuo ->
                                acopioCliente.registrarResiduo(
                                        residuo.getTipo().name(),
                                        new AcopioTemporalClienteRest.RegistrarResiduoRequest(
                                                ordenId,
                                                residuo.getId(),
                                                residuo.getVolumen().getValor(),
                                                residuo.getVolumen().getUnidad(),
    ...
    private PrecioAplicado precio(OrdenServicioClienteRest.PrecioResponse precio) {
        if (precio == null)
            throw new IllegalArgumentException("Todo concepto autorizado debe tener precio");
