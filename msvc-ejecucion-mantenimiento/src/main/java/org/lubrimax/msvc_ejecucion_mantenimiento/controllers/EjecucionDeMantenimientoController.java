...
public class EjecucionDeMantenimientoController {
    ...
    @PostMapping
    public ResponseEntity<EjecucionDeMantenimiento> crear(
            @Valid @RequestBody CrearEjecucionRequest request) {
        return ResponseEntity.status(HttpStatus.CREATED)
                .body(
                        service.crear(
                                request.ordenId(),
                                request.tecnicoUsuarioId(),
                                request.kilometraje()));
    }

    @GetMapping("/{ordenId}")
    public EjecucionDeMantenimiento obtener(@PathVariable Long ordenId) {
        return service.obtener(ordenId);
    }

    @GetMapping
    public List<EjecucionDeMantenimiento> listar() {
        return service.listar();
    }

    @PostMapping("/{ordenId}/iniciar")
    public EjecucionDeMantenimiento iniciar(@PathVariable Long ordenId) {
        return service.iniciar(ordenId);
    }

    @PostMapping("/{ordenId}/suspender")
    public EjecucionDeMantenimiento suspender(@PathVariable Long ordenId) {
        return service.suspender(ordenId);
    }

    @PostMapping("/{ordenId}/tareas")
    public EjecucionDeMantenimiento registrarTarea(
            @PathVariable Long ordenId, @Valid @RequestBody RegistrarTareaRequest request) {
        return service.registrarTarea(ordenId, request.servicioId(), request.tipo());
    }

    @PostMapping("/{ordenId}/tareas/{tareaId}/iniciar")
    public EjecucionDeMantenimiento iniciarTarea(
            @PathVariable Long ordenId, @PathVariable Long tareaId) {
        return service.iniciarTarea(ordenId, tareaId);
    }

    @PostMapping("/{ordenId}/tareas/{tareaId}/completar")
    public EjecucionDeMantenimiento completarTarea(
            @PathVariable Long ordenId, @PathVariable Long tareaId) {
        return service.completarTarea(ordenId, tareaId);
    }

    @PostMapping("/{ordenId}/tareas/{tareaId}/no-realizada")
    public EjecucionDeMantenimiento marcarNoRealizada(
            @PathVariable Long ordenId, @PathVariable Long tareaId) {
        return service.marcarTareaNoRealizada(ordenId, tareaId);
    }

    @PostMapping("/{ordenId}/consumos")
    public EjecucionDeMantenimiento registrarConsumo(
            @PathVariable Long ordenId, @Valid @RequestBody RegistrarConsumoRequest request) {
    ...
    public EjecucionDeMantenimiento declararResiduo(
            @PathVariable Long ordenId, @Valid @RequestBody DeclararResiduoRequest request) {
        return service.declararResiduo(
                ordenId,
                request.tareaId(),
                request.tipo(),
    ...
    public EjecucionDeMantenimiento registrarObservacion(
            @PathVariable Long ordenId, @Valid @RequestBody RegistrarObservacionRequest request) {
        return service.registrarObservacion(
                ordenId, new ObservacionPreventiva(request.tipo(), request.descripcion(), null));
    }
    ...
    public EjecucionDeMantenimiento definirProximoServicio(
            @PathVariable Long ordenId, @Valid @RequestBody ProximoServicioRequest request) {
        return service.definirProximoServicio(
                ordenId,
                new ProgramacionDeProximoServicio(
                        request.fechaSugerida(), request.kilometrajeSugerido()));
    }

    @PostMapping("/{ordenId}/cerrar")
    public EjecucionDeMantenimiento cerrar(@PathVariable Long ordenId) {
        return service.cerrar(ordenId);
    }

    public record CrearEjecucionRequest(
            @NotNull Long ordenId,
            @NotNull Long tecnicoUsuarioId,
    ...
    public record DeclararResiduoRequest(
            @NotNull Long tareaId,
            @NotNull TipoDeResiduoGenerado tipo,
            @NotNull @Positive BigDecimal volumen,
            @NotBlank String unidad) {}

    public record RegistrarObservacionRequest(
            @NotBlank String tipo, @NotBlank String descripcion) {}

    public record ProximoServicioRequest(
            LocalDate fechaSugerida, @Positive Long kilometrajeSugerido) {}
}
