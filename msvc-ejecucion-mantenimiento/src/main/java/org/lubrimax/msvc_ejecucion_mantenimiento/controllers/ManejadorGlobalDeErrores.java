...
public class ManejadorGlobalDeErrores {
    ...
    public ResponseEntity<Map<String, Object>> manejar(
            RuntimeException exception, HttpServletRequest request) {
        return respuesta(HttpStatus.BAD_REQUEST, exception.getMessage(), request.getRequestURI());
    }

    @ExceptionHandler(FeignException.class)
    public ResponseEntity<Map<String, Object>> manejarFeign(
            FeignException exception, HttpServletRequest request) {
        return respuesta(
                HttpStatus.BAD_GATEWAY,
                "No se pudo completar la validacion con otro microservicio",
