...
public class EntregaAOperador {
    ...
    @Embedded
    private AutorizacionDelOperador autorizacionDelOperador;

    @Embedded
    private ManifiestoDeResiduos manifiesto;
    ...
    public EntregaAOperador(Long operadorId, Long usuarioResponsableId, LocalDate fechaEntrega) {
        if (operadorId == null) throw new IllegalArgumentException("El operador es obligatorio");
        if (usuarioResponsableId == null) throw new IllegalArgumentException("El usuario responsable es obligatorio");
        if (fechaEntrega == null) throw new IllegalArgumentException("La fecha de entrega es obligatoria");
    ...
    public LineaDeEntrega agregarLinea(TipoDeResiduo tipo, CantidadDeResiduo cantidad, List<Long> residuosIds) {
        exigirProgramada();
        boolean residuoRepetido = lineas.stream().flatMap(linea -> linea.getResiduosIds().stream())
                .anyMatch(residuosIds::contains);
        if (residuoRepetido) throw new IllegalArgumentException("Un residuo no puede aparecer en dos lineas");
    ...
    public void eliminarLinea(Long lineaId) {
        ...
        if (!eliminada) throw new IllegalArgumentException("No se encontro la linea con ID: " + lineaId);
    }

    public void registrarManifiesto(ManifiestoDeResiduos manifiesto) {
        exigirProgramada();
        if (manifiesto == null) throw new IllegalArgumentException("El manifiesto es obligatorio");
        this.manifiesto = manifiesto;
    }

    public void ejecutar(AutorizacionDelOperador autorizacion) {
        exigirProgramada();
        if (lineas.isEmpty()) throw new IllegalStateException("La entrega debe tener al menos una linea");
        if (manifiesto == null) throw new IllegalStateException("Debe registrar el manifiesto antes de ejecutar la entrega");
    ...
    public void conformar() {
        if (estado == EstadoDeEntrega.CONFORMADA) return;
    ...
    private void exigirProgramada() {
        if (estado != EstadoDeEntrega.PROGRAMADA) {
            throw new IllegalStateException("La operacion solo se permite mientras la entrega esta PROGRAMADA");
        }
    }
    ...
    public EstadoDeEntrega getEstado() { return estado; }
    public AutorizacionDelOperador getAutorizacionDelOperador() { return autorizacionDelOperador; }
    public ManifiestoDeResiduos getManifiesto() { return manifiesto; }
    public List<LineaDeEntrega> getLineas() { return Collections.unmodifiableList(lineas); }
}
