...
public class LineaDeEntrega {
    ...
    public LineaDeEntrega(TipoDeResiduo tipoDeResiduo, CantidadDeResiduo cantidad, List<Long> residuosIds) {
        ...
        if (residuosIds.stream().anyMatch(id -> id == null) || residuosIds.stream().distinct().count() != residuosIds.size()) {
            throw new IllegalArgumentException("Los residuos de la linea deben ser validos y no repetirse");
        }
    ...
    public List<Long> getResiduosIds() { return Collections.unmodifiableList(residuosIds); }
}
