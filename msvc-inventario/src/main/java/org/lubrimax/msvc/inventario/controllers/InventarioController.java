...
public class InventarioController {
    ...
    public ResponseEntity<?> buscarPorProductoPresentacion(@PathVariable Long productoId, @PathVariable Long presentacionId) {
        Optional<ExistenciaDeProducto> existenciaOptional = inventarioService.buscarPorProductoPresentacion(productoId, presentacionId);
        if (existenciaOptional.isPresent()) {
            return ResponseEntity.ok(existenciaOptional.get());
        }
        return ResponseEntity.notFound().build();
    ...
    // El evento 'StockInsumoHabilitado' de Abastecimiento te envía estos datos directo por parámetros HTTP
    ...
    public ResponseEntity<?> habilitarStock(
        ...
        try {
            ExistenciaDeProducto existencia = inventarioService.habilitarStock(
                    productoId,
                    presentacionId,
                    cantidad,
                    recepcionId
    ...
    public ResponseEntity<?> consumir(@PathVariable Long id, @RequestParam Long orden, @RequestParam BigDecimal cantidad) {

        try {
            ExistenciaDeProducto existencia =
                    inventarioService.consumir(
                            id,
                            orden,
                            cantidad
                    );
    ...
    public ResponseEntity<?> eliminar(@PathVariable Long id) {
        ...
        if (existenciaOpt.isPresent()) {
            ...
            if (fisico.compareTo(java.math.BigDecimal.ZERO) > 0 || reservado.compareTo(java.math.BigDecimal.ZERO) > 0) {
                return ResponseEntity.badRequest().body(Map.of(
                        "error", "No se puede eliminar la existencia: El almacén registra saldos o reservas activas para este producto."
                ));
            }
    ...
    public ResponseEntity<?> confirmarReserva(@PathVariable Long id, @RequestParam Long orden) {
        ...
        try {
            ExistenciaDeProducto existencia =
                    inventarioService.confirmarReserva(
                            id,
                            orden
                    );
            return ResponseEntity.ok(existencia);
