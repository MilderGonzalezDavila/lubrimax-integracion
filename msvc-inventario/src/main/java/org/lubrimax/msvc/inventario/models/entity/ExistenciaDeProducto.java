...
@Entity
@Table(name = "existencias", uniqueConstraints = { @UniqueConstraint(columnNames = {"producto_id", "presentacion_id"})})
public class ExistenciaDeProducto {
    ...
    public void consumir(Long orden, BigDecimal cantidad) {
        ...
        if (reservaConfirmada.isPresent()) {
            ...
            BigDecimal reservadoDeEstaOrden =
                    reserva.getCantidad();

            BigDecimal reservadoDeOtrasOrdenes =
                    this.saldo.getReservado()
                            .subtract(reservadoDeEstaOrden);

            BigDecimal maximoConsumible =
                    this.saldo.getFisico()
                            .subtract(reservadoDeOtrasOrdenes);
            ...
            this.saldo.restarReservado(
                    reservadoDeEstaOrden
            );
    ...
    public void reservar(
        ...
        boolean yaExisteReservaActiva = this.reservas.stream()
                .anyMatch(r ->
                        r.getOrden().equals(orden)
                                && (
        ...
        if (cantidad.compareTo(disponible()) > 0) {
            throw new IllegalArgumentException(
                    "No existe stock suficiente para reservar"
            );
        }

        ReservaTemporal reserva =
                new ReservaTemporal(orden,
                        cantidad,
                        expiraEn
                );
    ...
    public void confirmarReserva(Long orden) {
        ...
        ReservaTemporal reserva = this.reservas.stream()
                .filter(r ->
                        r.getOrden().equals(orden)
                                && r.getEstado() == EstadoReserva.VIGENTE
                )
    ...
    public void liberarReserva(Long orden) {
        ...
        ReservaTemporal reserva = this.reservas.stream()
                .filter(r -> r.getOrden().equals(orden)
                        && r.getEstado() == EstadoReserva.VIGENTE)
                .findFirst()
                .orElseThrow(() ->
                        new IllegalArgumentException(
                                "No existe una reserva vigente para la orden: " + orden
                        )
                );
    ...
    public SaldoDeAlmacen getSaldo() { return saldo; }
    public void setSaldo(SaldoDeAlmacen saldo) { this.saldo = saldo; }

    public List<ReservaTemporal> getReservas() { return reservas; }
    public void setReservas(List<ReservaTemporal> reservas) { this.reservas = reservas; }

    public List<MovimientoDeStock> getMovimientos() { return movimientos; }
    public void setMovimientos(List<MovimientoDeStock> movimientos) { this.movimientos = movimientos; }
}