...
public class AgendaDelTecnico {
    ...
    public AgendaDelTecnico(Long usuarioId) {
        if (usuarioId == null) {
            throw new IllegalArgumentException("El usuario tecnico es obligatorio");
        }
        this.usuarioId = usuarioId;
    }

    public AsignacionDeTrabajo asignar(
            Long ordenId, PeriodoDeTrabajo periodo, boolean tecnicoActivo) {
        if (!tecnicoActivo) {
            throw new IllegalArgumentException("El usuario no tiene un rol de tecnico activo");
        }
        if (!estaDisponible(periodo)) {
            throw new IllegalStateException(
                    "El tecnico ya tiene una asignacion que se solapa con ese periodo");
        }
    ...
    public boolean estaDisponible(PeriodoDeTrabajo periodo) {
        return asignaciones.stream()
                .noneMatch(
                        asignacion ->
                                asignacion.ocupaElPeriodo()
                                        && asignacion.getPeriodo().seSolapaCon(periodo));
    }
    ...
    public boolean tieneAsignacionValida(Long ordenId) {
        return asignaciones.stream()
                .anyMatch(
                        asignacion ->
                                ordenId.equals(asignacion.getOrdenId())
                                        && asignacion.getEstado() != EstadoDeAsignacion.LIBERADA);
    }

    private AsignacionDeTrabajo buscarAsignacion(Long asignacionId) {
        return asignaciones.stream()
                .filter(asignacion -> asignacion.getId().equals(asignacionId))
                .findFirst()
                .orElseThrow(
                        () ->
                                new IllegalArgumentException(
                                        "No se encontro la asignacion con ID: " + asignacionId));
    }
    ...
    public List<AsignacionDeTrabajo> getAsignaciones() {
